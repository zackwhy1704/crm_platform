...
Ported from multi_platform_automation/gateway/whatsapp_client.py (April 2026).
Provides async + sync send methods for text, interactive buttons, interactive lists,
media upload, and image/video sending.

Lead qualification in this project uses interactive buttons heavily to get
structured BANT answers without relying on NLP to parse "yes/yeah/can/ok lah".

Set FAKE_WHATSAPP=true in env to log messages instead of sending — use during M1
local dev before wiring a real Meta test number.
"""
...
def _get_url() -> str:
    ...
    from app.config import WHATSAPP_PHONE_NUMBER_ID as _phone_id
    return f"https://graph.facebook.com/{GRAPH_API_VERSION}/{_phone_id}/messages"


def _get_headers() -> dict:
    from app.config import WHATSAPP_TOKEN as _token
    return {
...
async def send_interactive_buttons(to: str, body: str, buttons: list[dict]) -> bool:
    ...
    We use this for BANT questions:
      - Ownership: [Own] [Rent]
      - Timeline: [Within 3 months] [3-6 months] [Just exploring]
    """
    btn_rows = [
...
async def send_interactive_list(
    ...
    """
    Send an interactive list message (more than 3 options).

    sections: [{"title": "Section", "rows": [{"id": "row1", "title": "Item", "description": "..."}]}]

    We use this for:
      - Property type: HDB 3-room / 4-room / 5-room / EC / Condo / Landed
      - Budget range: <10k / 10-30k / 30-60k / 60k+
    """
    payload = {
        "messaging_product": "whatsapp",
        "to": to,
...
async def upload_media(file_path: str, mime_type: str) -> Optional[str]:
    ...
    Uploaded media IDs are valid for 30 days and render instantly.
    """
    from app.config import WHATSAPP_PHONE_NUMBER_ID as _phone_id
    from app.config import WHATSAPP_TOKEN as _token
    import os
...
async def send_image(
    to: str, image_url: str, caption: str = "",
    file_path: str = "", mime_type: str = "",
) -> bool:
    """Send an image. Prefers direct upload (file_path) over URL for instant rendering.

    Use case: sending portfolio photos to qualified leads or client-requested images.
    """
    media_id = None
...
async def send_video(
    to: str, video_url: str, caption: str = "",
    file_path: str = "", mime_type: str = "",
) -> bool:
    """Send a video. Prefers direct upload over URL."""
    media_id = None
    if file_path and mime_type:
        media_id = await upload_media(file_path, mime_type)
...
async def _post(payload: dict) -> bool:
    ...
    if FAKE_WHATSAPP:
        logger.info("[FAKE_WHATSAPP] would send: %s", payload)
        return True
